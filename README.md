...
Para la instalación usamos el role [ansible-role-nfs](https://github.com/indigo-dc/ansible-role-nfs)

Para actualizarlo podemos hacerlo con ansible-galaxy


```
$ ansible-galaxy install git+https://github.com/indigo-dc/ansible-role-nfs
```

Lanzar el ansible crear el servidor y cliente

```
$ ansible-playbook playbooks/install_nfs-client.yml  \
 -i inventories \
 -k -K \
 --vault-password-file vault \
 -u ubuntu

 $ ansible-playbook playbooks/install_nfs-server.yml  \
  -i inventories \
  -k -K \
  --vault-password-file vault \
  -u ubuntu
